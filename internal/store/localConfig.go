...
// GenPConfig represents the full structure of the genp.yaml config file.
// It holds both the GitHub OAuth token and the stored encrypted passwords.
type GenPConfig struct {
	Token    string            `yaml:"token,omitempty"`
	Password map[string]string `yaml:"password,omitempty"`
}
...
// configFilePath returns the full path to the genp.yaml config file.
func configFilePath() (string, error) {
	...
	return filepath.Join(baseDir, "genp.yaml"), nil
}

// readFullConfig reads and parses the entire genp.yaml into a GenPConfig struct.
// If the file does not exist, it returns an empty config (not an error).
func readFullConfig() (*GenPConfig, error) {
...
// writeFullConfig marshals a GenPConfig and writes it to genp.yaml,
// creating the directory if needed.
func writeFullConfig(cfg *GenPConfig) error {
	confPath, err := configFilePath()
	if err != nil {
		return err
	}
...
// SaveToken stores the GitHub OAuth token in genp.yaml alongside the passwords.
func SaveToken(token string) error {
	cfg, err := readFullConfig()
	if err != nil {
		return fmt.Errorf("failed to read config for saving token: %w", err)
	}
...
// GetToken retrieves the GitHub OAuth token from genp.yaml.
// Returns an error if the token is not found or the file doesn't exist.
func GetToken() (string, error) {
	cfg, err := readFullConfig()
	if err != nil {
		return "", fmt.Errorf("failed to read config for token: %w", err)
	}

	if cfg.Token == "" {
		return "", fmt.Errorf("not logged in. Run `genp login` first")
	}

	return cfg.Token, nil
}

// RemoveToken removes the GitHub OAuth token from genp.yaml,
// leaving passwords intact.
func RemoveToken() error {
	cfg, err := readFullConfig()
	if err != nil {
		return fmt.Errorf("failed to read config for removing token: %w", err)
	}

	if cfg.Token == "" {
		return fmt.Errorf("no token found; you are not logged in")
	}
